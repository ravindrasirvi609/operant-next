...
async function upsertTotalEntries(form: InstanceType<typeof FacultyPbasForm>) {
    ...
    const scoreByCode: Record<string, number> = {
        A_TOTAL: Number(form.apiScore?.teachingActivities ?? 0),
        B_TOTAL: Number(form.apiScore?.researchAcademicContribution ?? 0),
        C_TOTAL: Number(form.apiScore?.institutionalResponsibilities ?? 0),
        API_TOTAL: Number(form.apiScore?.totalScore ?? 0),
    };

    const isLocked = form.submissionStatus === "Locked" || form.status === "Approved";
    ...
    await Promise.all(
        Object.entries(scoreByCode).map(async ([indicatorCode, claimedScore]) => {
            ...
            await FacultyPbasEntry.updateOne(
                { pbasFormId: form._id, indicatorId },
                {
                    $set: {
                        pbasFormId: form._id,
                        indicatorId,
...
export async function syncPbasTotalEntries(formId: string) {
