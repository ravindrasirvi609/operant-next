...
type PlanFormState = {
    ...
    summary: string;
    strategicPriorities: string;
    targetMeetingCount: string;
    targetInitiativeCount: string;
    targetPolicyCount: string;
    targetComplianceReviewCount: string;
    ownerUserId: string;
    status: string;
};
...
const emptyPlanForm: PlanFormState = {
    ...
    summary: "",
    strategicPriorities: "",
    targetMeetingCount: "0",
    targetInitiativeCount: "0",
    targetPolicyCount: "0",
    targetComplianceReviewCount: "0",
    ownerUserId: "",
    status: "Draft",
};
...
export function GovernanceLeadershipIqacManager({
    plans,
    assignments,
    academicYearOptions,
    departmentOptions,
    ...
    useEffect(() => {
        ...
        setPlanForm({
            ...
            targetMeetingCount: String(plan.targetMeetingCount ?? 0),
            targetInitiativeCount: String(plan.targetInitiativeCount ?? 0),
            targetPolicyCount: String(plan.targetPolicyCount ?? 0),
            targetComplianceReviewCount: String(plan.targetComplianceReviewCount ?? 0),
        ...
        startTransition(async () => {
            try {
                const payload = {
                    ...
                    targetMeetingCount: Number(planForm.targetMeetingCount || 0),
                    targetInitiativeCount: Number(planForm.targetInitiativeCount || 0),
                    targetPolicyCount: Number(planForm.targetPolicyCount || 0),
                    targetComplianceReviewCount: Number(planForm.targetComplianceReviewCount || 0),
                ...
                const data = await requestJson<{ message?: string }>(
                    editingPlanId
                        ? `/api/admin/governance-leadership-iqac/plans/${editingPlanId}`
                        : "/api/admin/governance-leadership-iqac/plans",
                    {
                ...
                setMessage({
                    type: "success",
                    text: data.message ?? "Governance Leadership / IQAC plan saved successfully.",
                });
                resetPlanForm();
                router.refresh();
            } catch (error) {
                setMessage({
                    ...
                    text:
                        error instanceof Error
                            ? error.message
                            : "Unable to save the governance leadership / IQAC plan.",
                });
            }
        });
    }
    ...
    function submitAssignment() {
        ...
        startTransition(async () => {
            try {
                ...
                const data = await requestJson<{ message?: string }>(
                    editingAssignmentId
                        ? `/api/admin/governance-leadership-iqac/assignments/${editingAssignmentId}`
                        : "/api/admin/governance-leadership-iqac/assignments",
                    {
                ...
                setMessage({
                    type: "success",
                    text: data.message ?? "Governance Leadership / IQAC assignment saved successfully.",
                });
                resetAssignmentForm();
                router.refresh();
            } catch (error) {
                setMessage({
                    ...
                    text:
                        error instanceof Error
                            ? error.message
                            : "Unable to save the governance leadership / IQAC assignment.",
                });
            }
        });
    }
                ...
                <div className="grid gap-6 xl:grid-cols-[380px_1fr]">
                    <Card>
                        <CardHeader>
                            ...
                            <CardDescription>
                                Create the governed leadership and IQAC portfolio and set the eligible scoped owner.
                            </CardDescription>
                        </CardHeader>
                        <CardContent className="space-y-4">
                            ...
                            <div className="space-y-2">
                                ...
                                <Input
                                    ...
                                    placeholder="Governance Leadership / IQAC plan 2025-26"
                            ...
                            <div className="space-y-2">
                                ...
                                >
                                    <SelectTrigger>
                                        <SelectValue placeholder="Select eligible leadership or IQAC owner" />
                                    </SelectTrigger>
                                    <SelectContent>
                            ...
                            <div className="grid gap-3 md:grid-cols-2">
                                <TargetField label="IQAC meetings" value={planForm.targetMeetingCount} onChange={(value) => setPlanForm((current) => ({ ...current, targetMeetingCount: value }))} />
                                <TargetField label="Quality initiatives" value={planForm.targetInitiativeCount} onChange={(value) => setPlanForm((current) => ({ ...current, targetInitiativeCount: value }))} />
                                <TargetField label="Policies / circulars" value={planForm.targetPolicyCount} onChange={(value) => setPlanForm((current) => ({ ...current, targetPolicyCount: value }))} />
                                <TargetField label="Compliance reviews" value={planForm.targetComplianceReviewCount} onChange={(value) => setPlanForm((current) => ({ ...current, targetComplianceReviewCount: value }))} />
                            </div>
                            ...
                            <div className="space-y-2">
                                ...
                                <Textarea
                                    onChange={(event) =>
                                        setPlanForm((current) => ({ ...current, strategicPriorities: event.target.value }))
                                    }
                                    rows={4}
                                    ...
                                    <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                                        <div>
                                            ...
                                            <p className="mt-2 text-xs text-zinc-500">
                                                Targets: {plan.targetMeetingCount} IQAC meetings, {plan.targetInitiativeCount} quality initiatives, {plan.targetPolicyCount} policies or circulars, {plan.targetComplianceReviewCount} compliance reviews
                                            </p>
                                        </div>
                                        <div className="space-y-2">
                                            <Button
                ...
                <div className="grid gap-6 xl:grid-cols-[380px_1fr]">
                    <Card>
                        <CardHeader>
                            ...
                            <CardDescription>
                                Server-side assignment enforcement only allows scoped IQAC, leadership, and governance actors.
                            </CardDescription>
                        </CardHeader>
                        <CardContent className="space-y-4">
                            <div className="space-y-2">
                            ...
                            <div className="space-y-2">
                                ...
                                >
                                    <SelectTrigger>
                                        <SelectValue placeholder="Select eligible IQAC or leadership assignee" />
                                    </SelectTrigger>
                                    <SelectContent>
                                        {userOptions.map((option) => (
                                            <SelectItem key={option.id} value={option.id}>
                                ...
                                <p className="text-xs text-zinc-500">
                                    Enforcement happens in the service layer for scoped plan owners, IQAC members, leadership assignees, office heads, and committee members.
                                </p>
                            </div>
