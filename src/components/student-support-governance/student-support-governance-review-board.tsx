...
const narrativeSections: Array<{ key: keyof ReviewRecord; label: string }> = [
    { key: "mentoringFramework", label: "Mentoring framework" },
    { key: "grievanceRedressalSystem", label: "Grievance redressal system" },
    { key: "scholarshipSupport", label: "Scholarship support" },
    { key: "progressionTracking", label: "Progression tracking" },
    { key: "placementReadiness", label: "Placement readiness" },
    { key: "studentRepresentation", label: "Student representation" },
    { key: "wellbeingSupport", label: "Wellbeing support" },
    { key: "inclusionSupport", label: "Inclusion support" },
    { key: "feedbackMechanism", label: "Feedback mechanism" },
...
export function StudentSupportGovernanceReviewBoard({
    summary,
    records,
    viewerLabel,
}: {
        ...
        startTransition(async () => {
            try {
                const data = await requestJson<{ message?: string }>(
                    `/api/student-support-governance/assignments/${selectedRecord._id}/review`,
                    {
                        method: "POST",
                        body: JSON.stringify({
                            decision,
                ...
                setMessage({
                    ...
                    text:
                        data.message ??
                        "Student support & governance review recorded successfully.",
                });
                setRemarks("");
                router.refresh();
            } catch (error) {
                setMessage({
                    ...
                    text:
                        error instanceof Error
                            ? error.message
                            : "Unable to record the student support & governance review.",
                });
            }
        });
    }
                    ...
                    {filteredRecords.length ? (
                        ...
                        <Card>
                            <CardContent className="p-6 text-sm text-zinc-500">
                                No student-support records match the current filter.
                            </CardContent>
                        </Card>
                    )}
                </div>
                    ...
                    <div className="space-y-6">
                        <div className="rounded-2xl border border-zinc-200 bg-zinc-50 p-5">
                            <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                                ...
                                <div className="grid gap-2 text-right text-sm text-zinc-500">
                                    <p>{selectedRecord.mentorGroups.length} mentor group row(s)</p>
                                    <p>{selectedRecord.grievances.length} grievance row(s)</p>
                                    <p>{selectedRecord.progressionRows.length} progression row(s)</p>
                                    <p>{selectedRecord.representationRows.length} representation row(s)</p>
                        ...
                        <Card>
                            ...
                            <CardContent className="space-y-4">
                                <div className="grid gap-3 md:grid-cols-3">
                                    <MetricCard label="Mentor groups target" value={selectedRecord.planTargets.mentorGroups} />
                                    <MetricCard label="Closure target" value={selectedRecord.planTargets.grievanceClosures} />
                                    <MetricCard label="Scholarship target" value={selectedRecord.planTargets.scholarshipBeneficiaries} />
                                    <MetricCard label="Placement target" value={selectedRecord.planTargets.placements} />
                                    <MetricCard label="Higher studies target" value={selectedRecord.planTargets.higherStudies} />
                                    <MetricCard label="Representation target" value={selectedRecord.planTargets.representationBodies} />
                                </div>
                        ...
                        <Card>
                            <CardHeader>
                                ...
                                <CardDescription>
                                    Review the contributor’s student-support narrative before evaluating evidence rows.
                                </CardDescription>
                            </CardHeader>
                        ...
                        <RowCard
                            title="Mentor Groups"
                            emptyLabel="No mentor groups were added."
                            rows={selectedRecord.mentorGroups.map((row) => (
                                <div className="rounded-lg border border-zinc-200 bg-zinc-50 p-4" key={row.id}>
                                    <p className="text-sm font-semibold text-zinc-950">{row.groupName}</p>
                                    <p className="mt-1 text-xs text-zinc-500">
                                        {[row.programName, row.batchLabel, row.mentorName]
                                            .filter(Boolean)
                                            .join(" · ")}
                                    </p>
                                    <p className="mt-2 text-sm text-zinc-600">
                                    ...
                                    {row.supportThemes ? (
                                        <p className="mt-2 text-sm text-zinc-600">Themes: {row.supportThemes}</p>
                                    ) : null}
                                    {row.actionTaken ? (
                                        <p className="mt-2 text-sm text-zinc-600">Action: {row.actionTaken}</p>
                                    ) : null}
                                    {row.remarks ? (
                        ...
                        <RowCard
                            title="Grievances"
                            emptyLabel="No grievances were added."
                            rows={selectedRecord.grievances.map((row) => (
                                <div className="rounded-lg border border-zinc-200 bg-zinc-50 p-4" key={row.id}>
                                    <p className="text-sm font-semibold text-zinc-950">{row.category}</p>
                                    <p className="mt-1 text-xs text-zinc-500">
                                        {[row.referenceNumber, row.lodgedByType, row.status].filter(Boolean).join(" · ")}
                                    </p>
                                    <p className="mt-2 text-sm text-zinc-600">
                                        Received {formatDate(row.receivedDate)} · Resolved {formatDate(row.resolvedDate)} · Resolution days {row.resolutionDays ?? "-"}
                                    </p>
                                    {row.committeeName ? (
                                        <p className="mt-2 text-sm text-zinc-600">Committee: {row.committeeName}</p>
                                    ) : null}
                                    {row.resolutionSummary ? (
                                        <p className="mt-2 text-sm text-zinc-600">{row.resolutionSummary}</p>
                                    ) : null}
                                    {row.remarks ? (
                                        <p className="mt-2 text-sm text-zinc-600">{row.remarks}</p>
                                    ) : null}
                        ...
                        <RowCard
                            ...
                            emptyLabel="No progression rows were added."
                            rows={selectedRecord.progressionRows.map((row) => (
                                <div className="rounded-lg border border-zinc-200 bg-zinc-50 p-4" key={row.id}>
                                    <p className="text-sm font-semibold text-zinc-950">{row.title}</p>
                                    <p className="mt-1 text-xs text-zinc-500">
                                        {[row.progressionType, row.programName, row.batchLabel, row.status]
                                            .filter(Boolean)
                                            .join(" · ")}
                                    </p>
                                    <p className="mt-2 text-sm text-zinc-600">
                        ...
                        <RowCard
                            ...
                            emptyLabel="No representation rows were added."
                            rows={selectedRecord.representationRows.map((row) => (
                                <div className="rounded-lg border border-zinc-200 bg-zinc-50 p-4" key={row.id}>
                                    <p className="text-sm font-semibold text-zinc-950">{row.bodyName}</p>
                                    <p className="mt-1 text-xs text-zinc-500">
                                        {[row.representationType, row.roleTitle]
                                            .filter(Boolean)
                                            .join(" · ")}
                                    </p>
                                    <p className="mt-2 text-sm text-zinc-600">
                                    ...
                                    {row.outcomeSummary ? (
                                        <p className="mt-2 text-sm text-zinc-600">{row.outcomeSummary}</p>
                                    ) : null}
                                    {row.remarks ? (
                                        <p className="mt-2 text-sm text-zinc-600">{row.remarks}</p>
                                    ) : null}
                        ...
                        {selectedRecord.permissions.canReview ? (
                            <Card>
                                <CardHeader>
                                    ...
                                    <CardDescription>
                                        Capture your governed review decision for this student-support portfolio.
                                    </CardDescription>
                                </CardHeader>
                                <CardContent className="space-y-4">
