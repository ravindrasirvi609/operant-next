...
type PlanFormState = {
    ...
    summary: string;
    strategyGoals: string;
    targetMentorGroupCount: string;
    targetGrievanceClosureCount: string;
    targetScholarshipBeneficiaryCount: string;
    targetPlacementCount: string;
    targetHigherStudiesCount: string;
    targetRepresentationCount: string;
    facultyOwnerUserId: string;
    status: string;
};
...
const emptyPlanForm: PlanFormState = {
    ...
    summary: "",
    strategyGoals: "",
    targetMentorGroupCount: "0",
    targetGrievanceClosureCount: "0",
    targetScholarshipBeneficiaryCount: "0",
    targetPlacementCount: "0",
    targetHigherStudiesCount: "0",
    targetRepresentationCount: "0",
    facultyOwnerUserId: "",
    status: "Draft",
};
...
export function StudentSupportGovernanceManager({
    ...
    useEffect(() => {
        ...
        setPlanForm({
            ...
            targetMentorGroupCount: String(plan.targetMentorGroupCount ?? 0),
            targetGrievanceClosureCount: String(plan.targetGrievanceClosureCount ?? 0),
            targetScholarshipBeneficiaryCount: String(plan.targetScholarshipBeneficiaryCount ?? 0),
            targetPlacementCount: String(plan.targetPlacementCount ?? 0),
            targetHigherStudiesCount: String(plan.targetHigherStudiesCount ?? 0),
            targetRepresentationCount: String(plan.targetRepresentationCount ?? 0),
        ...
        startTransition(async () => {
            try {
                const payload = {
                    ...
                    targetMentorGroupCount: Number(planForm.targetMentorGroupCount || 0),
                    targetGrievanceClosureCount: Number(planForm.targetGrievanceClosureCount || 0),
                    targetScholarshipBeneficiaryCount: Number(planForm.targetScholarshipBeneficiaryCount || 0),
                    targetPlacementCount: Number(planForm.targetPlacementCount || 0),
                    targetHigherStudiesCount: Number(planForm.targetHigherStudiesCount || 0),
                    targetRepresentationCount: Number(planForm.targetRepresentationCount || 0),
                ...
                const data = await requestJson<{ message?: string }>(
                    editingPlanId
                        ? `/api/admin/student-support-governance/plans/${editingPlanId}`
                        : "/api/admin/student-support-governance/plans",
                    {
                ...
                setMessage({
                    type: "success",
                    text: data.message ?? "Student support & governance plan saved successfully.",
                });
                resetPlanForm();
                router.refresh();
            } catch (error) {
                setMessage({
                    ...
                    text:
                        error instanceof Error
                            ? error.message
                            : "Unable to save the student support & governance plan.",
                });
            }
        });
    }
    ...
    function submitAssignment() {
        ...
        startTransition(async () => {
            try {
                ...
                const data = await requestJson<{ message?: string }>(
                    editingAssignmentId
                        ? `/api/admin/student-support-governance/assignments/${editingAssignmentId}`
                        : "/api/admin/student-support-governance/assignments",
                    {
                ...
                setMessage({
                    type: "success",
                    text: data.message ?? "Student support & governance assignment saved successfully.",
                });
                resetAssignmentForm();
                router.refresh();
            } catch (error) {
                setMessage({
                    ...
                    text:
                        error instanceof Error
                            ? error.message
                            : "Unable to save the student support & governance assignment.",
                });
            }
        });
    }
                ...
                <div className="grid gap-6 xl:grid-cols-[380px_1fr]">
                    <Card>
                        <CardHeader>
                            ...
                            <CardDescription>
                                Create the governed student-support portfolio and set the eligible faculty owner.
                            </CardDescription>
                        </CardHeader>
                        <CardContent className="space-y-4">
                            ...
                            <div className="space-y-2">
                                ...
                                <Input
                                    ...
                                    placeholder="Student support & governance plan 2025-26"
                            ...
                            <div className="space-y-2">
                                ...
                                >
                                    ...
                                    <SelectContent>
                                        ...
                                        <SelectItem value="StudentSupport">Student Support</SelectItem>
                            ...
                            <div className="grid gap-3 md:grid-cols-2">
                                <TargetField label="Mentor groups" value={planForm.targetMentorGroupCount} onChange={(value) => setPlanForm((current) => ({ ...current, targetMentorGroupCount: value }))} />
                                <TargetField label="Grievance closures" value={planForm.targetGrievanceClosureCount} onChange={(value) => setPlanForm((current) => ({ ...current, targetGrievanceClosureCount: value }))} />
                                <TargetField label="Scholarship beneficiaries" value={planForm.targetScholarshipBeneficiaryCount} onChange={(value) => setPlanForm((current) => ({ ...current, targetScholarshipBeneficiaryCount: value }))} />
                                <TargetField label="Placements" value={planForm.targetPlacementCount} onChange={(value) => setPlanForm((current) => ({ ...current, targetPlacementCount: value }))} />
                                <TargetField label="Higher studies" value={planForm.targetHigherStudiesCount} onChange={(value) => setPlanForm((current) => ({ ...current, targetHigherStudiesCount: value }))} />
                                <TargetField label="Representation bodies" value={planForm.targetRepresentationCount} onChange={(value) => setPlanForm((current) => ({ ...current, targetRepresentationCount: value }))} />
                            </div>
                ...
                <div className="grid gap-6 xl:grid-cols-[380px_1fr]">
                    <Card>
                        <CardHeader>
                            ...
                            <CardDescription>
                                Server-side assignment enforcement only allows scoped student-support coordinators and plan owners.
                            </CardDescription>
                        </CardHeader>
                        <CardContent className="space-y-4">
                            <div className="space-y-2">
